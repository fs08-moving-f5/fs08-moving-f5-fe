  ...
  const renderStars = () => {
    return Array.from({ length: 5 }, (_, index) => {
      ...
      if (fillPercentage === 100) {
        return <Image src="/icons/star.svg" alt="star" width={size} height={size} key={index} />;
      } else if (fillPercentage === 0) {
        return (
          <Image src="/icons/star-empty.svg" alt="star" width={size} height={size} key={index} />
        );
      } else {
        // 부분적으로 채워진 별
        return (
